...
for key in country_dates:
    ...
    for day in percentage_curve:
        day = float(day) * population/4814221 * 81
        cases_pc = float(day)
        ...
        if death_date < 10:
            ...
            deaths = float(percentage_curve[death_date-10]) * population/4814221 * 81 * population  * 0.006
            total_deaths += deaths
    ...
    for day in death_remain:
        deaths =  float(day) * population/4814221 * 81 * population * 0.006
        total_deaths += deaths
        deaths_pc = deaths / population
        total_deaths_pc = total_deaths / population
        str_date = date.strftime("%Y-%m-%d") 
