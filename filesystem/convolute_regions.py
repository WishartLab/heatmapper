...
def Extract(lst, index):
    lst.remove("N/A")
    return [float(item[index]) for item in lst[1:]] 
...
italy_curve = np.array(Extract(list(italy_data),5))
...
for region, name in cases:
    ...
    region_confirmed = np.convolve(medfilt(np.diff(region_confirmed)), np.ones((3,))/3, mode='valid')
    region_confirmed = np.convolve(medfilt(region_confirmed), np.ones((3,))/3, mode='valid')

    region_curve = derivative_slope(active_cases)
    region_curve = np.convolve(medfilt(active_cases), np.ones((3,))/3, mode='valid')
    ...
    plt.show()
    plt.plot(np.array(get_day_count(region_confirmed,0)), np.array(region_confirmed), 'k--', label='actual')
    plt.plot(np.array(get_day_count(region_projected,len(region_confirmed))), np.array(region_projected), 'r-', label='projected')
    plt.xlabel("Day",fontsize=18)
